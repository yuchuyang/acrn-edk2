...
{
  IoWrite8 (PcdGet8 (PcdRtcIndexRegister), Address);
...
/**
  Initialize RTC. Gets the timezone and daylight information and exports
  to RTC module usage. All the rest of RTC control registers and time
  registers are initialized by hypervisor.
  ...
  )
{
  ...
  if ((CenturyRtcAddress != 0) && (mModuleGlobal.CenturyRtcAddress != CenturyRtcAddress)) {
    ...
    Status = AcrnRtcGetTime (&Time, NULL, &mModuleGlobal);
    if (!EFI_ERROR (Status)) {
      Century = (UINT8) (Time.Year / 100);
      Century = DecimalToBcd8 (Century);
      DEBUG ((EFI_D_INFO, "AcrnRtc: Write 0x%x to CMOS location 0x%x\n", Century, mModuleGlobal.CenturyRtcAddress));
