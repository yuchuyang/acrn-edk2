  ...
  )
{
  ...
  if ((FwVolHeader->Revision != EFI_FVH_REVISION) ||
    ...
    DEBUG ((EFI_D_INFO, "AcrnEmu Variable FVB: Basic FV headers were invalid\n"));
  ...
  if (Checksum != 0) {
    DEBUG ((EFI_D_INFO, "AcrnEmu Variable FVB: FV checksum was invalid\n"));
    return EFI_NOT_FOUND;
  }
  ...
  )
{
  ...
  DEBUG ((EFI_D_INFO, "AcrnEmu Variable FVB Started\n"));
  ...
  if (
    ...
    DEBUG ((EFI_D_ERROR, "AcrnEmu Variable invalid PCD sizes\n"));
    return EFI_INVALID_PARAMETER;
  }
  ...
  //
  // Directly use the NV storage area as RAM
  //
  Ptr = (VOID *) (UINTN) PcdGet32 (PcdOvmfFlashNvStorageVariableBase);
  DEBUG ((
    EFI_D_INFO,
    "AcrnEmu Variable FVB: Using block at %p\n",
    Ptr
    ));
  Status = ValidateFvHeader (Ptr);
  if (!EFI_ERROR (Status)) {
    DEBUG ((EFI_D_INFO, "AcrnEmu Variable FVB: Found valid pre-existing FV\n"));
    Initialize = FALSE;
  }
  ...
  //
  DEBUG ((EFI_D_INFO, "Installing FVB for AcrnEmu Variable support\n"));
  Handle = 0;
